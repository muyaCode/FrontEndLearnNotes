...
节内容应该是本系列文章内容的第一篇，介绍如何搭建Tauri 2.0的开发环境，以及创建Tauri 2.0 的项目。
...
> Tauri 2.0 目前有个相当离谱的坑就是依赖问题，pnpm能解决js或ts的依赖问题，但是pnpm只能让依赖保持最新，并且还有缓存，寻找问题的原因变得极为困难，这就导致会出现一些莫名其妙的问题，因为Tauri版本与插件版本会出现不兼容问题。
>
...
![image](./Tauri2移动端开发和打包.assets/urc4jn6zkhw3i_49b8a9978b04470bad761a2ef8a2bc5c.png)

![image](./Tauri2移动端开发和打包.assets/urc4jn6zkhw3i_c195603917584ef1b87301a397554e44.png)
...
> 官方也提供了其他方式创建项目，如果你有兴趣，可以去官网看看[ ✈飞机票](https://tauri.app/)
>
> ![image](./Tauri2移动端开发和打包.assets/urc4jn6zkhw3i_2f0e2b7067dd4602ab800ee712288ba4.png)
>
> 执行那条命令，以下是输入的选项：
...
![image](./Tauri2移动端开发和打包.assets/urc4jn6zkhw3i_7691bc30f0344ee68f324ceb9e40ae96.png)
...
![image](./Tauri2移动端开发和打包.assets/urc4jn6zkhw3i_73c17d786437488e9686263be2606187.png)
...
![image](./Tauri2移动端开发和打包.assets/urc4jn6zkhw3i_65f75868dea8403c8f194148bd4cfc55.png)

![image](./Tauri2移动端开发和打包.assets/urc4jn6zkhw3i_dd0a20706b00441888ff17a26cd39e44.png)

编译完成后，效果如下

![image](./Tauri2移动端开发和打包.assets/urc4jn6zkhw3i_6457c8659e1d4cbbb66628b702a6c420.png)

## 三、设置开发环境

无论是Tauri还是Rust，官方都建议使用VSCode来进行开发，同时对其支持也是相当完善，因此使用VSCode做开发。安装好VSCode后，只需要安装以下安装插件

![image](./Tauri2移动端开发和打包.assets/urc4jn6zkhw3i_12e6790a37ad4d5ca99d9ec2bbaad341.png)

只要安装这三款插件即可

![image](./Tauri2移动端开发和打包.assets/urc4jn6zkhw3i_14a2aa07a13644b6958acd532075ec3b.png)

## 四、项目结构

当前项目文件结构如下

![image](./Tauri2移动端开发和打包.assets/urc4jn6zkhw3i_bc26532b17d241a59fab7ef991c7b885.png)
...
![image](./Tauri2移动端开发和打包.assets/urc4jn6zkhw3i_8591f159bc9149ec83624d73e099e9bd.png)

这个部分取决于你使用的是哪种前端，但本质上，还是前端那些东西，就不再赘述了。

但有一点是值得注意的

![image](./Tauri2移动端开发和打包.assets/urc4jn6zkhw3i_5d4d14f5967743b8b0968848614b7a6d.png)

此按钮功能的实现是前后端互动实现的，因此查其代码

![image](./Tauri2移动端开发和打包.assets/urc4jn6zkhw3i_3cda20ff2c61452b8a4b266d3d80d574.png)

![image](./Tauri2移动端开发和打包.assets/urc4jn6zkhw3i_da83e2cda6bd4783bd73e0562c1d6757.png)

此时查看后端文件

![image](./Tauri2移动端开发和打包.assets/urc4jn6zkhw3i_2db4f041038e4982b517ec825b3fd106.png)
...
其中`greet`就是前端所调用的那个函数，而`run`是程序的入口，有以下需要注意的
...
# tauri的安卓开发环境配置和编译.apk

### 环境安装

参考官方说明：链接：https://next--tauri.netlify.app/next/guides/getting-started/prerequisites/windows
...
- ![image6](./Tauri2移动端开发和打包.assets/image6.png)
...
- ![image5](./Tauri2移动端开发和打包.assets/image5.png)
...
- 然后再找到用户变量的`Path变量`，选择编辑。
  ![image4](./Tauri2移动端开发和打包.assets/image4.png)
...
- 参考截图如下：
  ![image3](./Tauri2移动端开发和打包.assets/image3.png)
...
### 构建Demo（这步的文档更新了，忽略以下，可按最新tauri2的文档）

参考页面链接：https://next--tauri.netlify.app/next/guides/getting-started/setup/html-css-js
...
3.打开`tauri_demo/src-tauri/pnpm.toml`文件，可以看到目前该项目依赖是Tauri 1.4，而不是Tauri 2.0。
...
- 由于目前tauri 2.0还是alpha状态，所以需要手动更新tauri 2.0。参考页面：https://next--tauri.netlify.app/next/mobile/development/configuration

1.进入到上面构建的tauri_demo目录，运行下面两个命令来升级tauri 2.0
...
```bash
pnpm add tauri@2.0.0-alpha.10
pnpm add tauri-build@2.0.0-alpha.6 --build
pnpm install tauri-cli --version "^2.0.0-alpha"
```

- 第一条命令运行出错了，`error: unrecognized feature for crate tauri: shell-open`
- 需要将`src-tauri/pnpm.toml`中的`features = ["shell-open"]`去掉，然后重新运行第一条命令。
...
Usage: pnpm tauri android [OPTIONS] <COMMAND>
...
- 然后发现`src-tauri/tauri.config.json` 中`allowlist`有波浪线提示，说这个属性无法解析，所以allow_list以及对应值都需要干掉。顺便将identityfier改成你的用户名，改完后长这样：
...
4.根据这个链接：https://next--tauri.netlify.app/next/mobile/development/integrate， 还需要做一些代码层面修改。

- 修改：`src-tauri/pnpm.toml`目录，末尾新增下面这段代码，让安卓可以使用静态/动态库。
...
![image2](./Tauri2移动端开发和打包.assets/image2.png)

#### 2.回到项目根路径，用tauri 2.0编译生成安卓代码

参考链接：https://next--tauri.netlify.app/next/mobile/development/mobile_workflow
...
参考链接：https://next--tauri.netlify.app/next/guides/distribution/sign-android/ 
...
![image](./Tauri2移动端开发和打包.assets/image.png)

# 环境配置2

## 文档

配置文档：[移动端配置](https://beta.tauri.app/zh-cn/guides/prerequisites/#%E7%A7%BB%E5%8A%A8%E7%AB%AF%E9%85%8D%E7%BD%AE)

能力：https://beta.tauri.app/zh-cn/concepts/security/capabilities/#target-platform

[访问控制列表](https://beta.tauri.app/zh-cn/references/v2/acl/#%E7%9B%AE%E6%A0%87%E5%B9%B3%E5%8F%B0)
...
![image](./Tauri2移动端开发和打包.assets/urc4jn6zkhw3i_249c52ad25a946d8bb63611d4edaf182.png)

## 二、接入SDK

点击接入SDK就会出现

![image](./Tauri2移动端开发和打包.assets/urc4jn6zkhw3i_7b686be9fffe45cf86d5c16774582169.png)然后在Tauri的src目录下（前端目录），找到`index.html`，添加这些内容
...
![image](./Tauri2移动端开发和打包.assets/urc4jn6zkhw3i_0fb7b8c096e74e109aa7db718a8d9aa6.png)
...
![image](./Tauri2移动端开发和打包.assets/urc4jn6zkhw3i_f6df3cdbb3c14e68a764a6521257eabb.png)表明已经连接到远程调试了，然后在**PageSpy**的Web页面进入房间页面，就会有你的设备

![image](./Tauri2移动端开发和打包.assets/urc4jn6zkhw3i_ed9c423941994fa3b82f238febeeb1a1.png)选中你的设备调试即可。

![image](./Tauri2移动端开发和打包.assets/urc4jn6zkhw3i_d090eb695add43febb07559c24ae11aa.png)
...
![image](./Tauri2移动端开发和打包.assets/urc4jn6zkhw3i_3df4658538954b359c60f4cb69ebf69b.png)

### 网络抓包

抓取请求响应，调试必备。

![image](./Tauri2移动端开发和打包.assets/urc4jn6zkhw3i_e592979417ea4236a6dc4c1067367ed9.png)

### 审查元素

![image](./Tauri2移动端开发和打包.assets/urc4jn6zkhw3i_48287454415d48aba2f7354be3b26f18.png)
