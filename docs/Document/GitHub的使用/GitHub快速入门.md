...
GitHub 文档：[GitHub Documentation](https://docs.github.com/cn)
...
`-u`作用：将来运行 git pull 命令从远程仓库获取内容时，本地仓库就可以直接从 origin 的 master 分支中获取内容，不需要添加其他的参数
...
master 分支是 Git 默认创建的分支，其他所有的分支都是在这个分支的基础上进行的。

- 不同的分支单独进行各自的作业

- 该分支的作业完成之后再和 master 分支合并
...
假设某个分支已经完成了作业需要和主分支 master 合并，使用如下语句：
