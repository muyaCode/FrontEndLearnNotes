...
在项目中新建perttier的默认配置文件： `.prettierrc` 文件，写入配置：
...
1. 三种打开设置方式：
   ...
   - `快捷键 `ctrl+shift+p` ，然后搜索：Settings -> 选择 -> 首选项：打开设置 -> 搜索Prettier
...
安装 [eslint-config-prettier](https://links.jianshu.com/go?to=https%3A%2F%2Fgithub.com%2Fprettier%2Feslint-config-prettier%23installation)，这个插件会把eslint中可能导致冲突的规则关掉，这样两者就能兼容使用了。
