...
在VitePress里面配置 algolia 搜索框，创建一个 `docs/.vitepress/algolia.js` 文件
...
在`docs/.vitepress/config.js`` 中导入引用 algolia的模块
...
2.复制公开的key：`Search-Only API Key` 到 vitepress项目的`docs/.vitepress/algolia.js`

appId字段填入：`Application ID`

apiKey字段填入：`Search-Only API Key`

indexName字段是：algolia刚刚创建的实例应用名称
...
3.执行ci配置，打开VitePress项目的github地址，点击Setting选项，点击 `Secrets` 中的 `Actions`。

![2-3.jpg](.\img\2-3.jpg)

3.1 创建 `API_KEY` 字段，将algolia应用的`Admin API Key` 复制到`API_KEY` 中

3.2 创建 `APPLICATION_ID` 字段，将algolia应用的 `Application ID` 复制到 `APPLICATION_ID`中

![2-4.jpg](.\img\2-4.jpg)

4.在VitePress项目中，创建 `crawlerConfig.json` 文件
