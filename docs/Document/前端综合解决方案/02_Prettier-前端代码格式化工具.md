...
项目根目录创建 [.prettierignore](https://links.jianshu.com/go?to=https%3A%2F%2Fprettier.io%2Fdocs%2Fen%2Fignore.html) 文件，忽略你不希望格式化的文件，node_modules 是默认会被忽略的目录。
...
- 安装 [eslint-config-prettier](https://links.jianshu.com/go?to=https%3A%2F%2Fgithub.com%2Fprettier%2Feslint-config-prettier%23installation)，这个插件会把 eslint 中可能导致冲突的规则关掉
...
需要安装[husky](https://links.jianshu.com/go?to=https%3A%2F%2Fgithub.com%2Ftypicode%2Fhusky) 和 [lint-staged](https://links.jianshu.com/go?to=https%3A%2F%2Fgithub.com%2Fokonet%2Flint-staged)这两个依赖才能实现，其中 husky 是帮助我们添加 git hooks 的工具，而 lint-staged 则是筛选那些 staged 的 git 文件执行 lint。
...
prettier 支持的配置文件的优先级如下，它使用 [cosmiconfig](https://links.jianshu.com/go?to=https%3A%2F%2Fgithub.com%2Fdavidtheclark%2Fcosmiconfig)提供配置文件支持的：
