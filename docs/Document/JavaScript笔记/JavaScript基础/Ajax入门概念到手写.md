...
> JS是单线程语言，所谓“单线程”就是一根筋，对于拿到的程序，一行一行的执行.
...
<body>
  ...
  <script>
    function sendMsg () {
      ...
      post('./data2.php', { id: 3, title: 'lisi' }, function (resp) {
        console.log(resp)
      }, true)
    }
...
2.封装的utils.js：ajax的get和post请求方法分别封装
...
- 段子列表和查看作者
  ...
  - 列表接口：[开放API-2.0](https://api.apiopen.top/getJoke)
  - 详情接口：[https://api.apiopen.top/getSingleJoke?sid=段子id](https://api.apiopen.top/getSingleJoke?sid=%E6%AE%B5%E5%AD%90id)
       ...
       <script>
         ...
         document.querySelector('#wrap').onclick = function (e) {
           ...
           if (target.tagName === 'BUTTON') {
             ...
             util.get('https://api.apiopen.top/getSingleJoke', {
               sid: sid
             }, function (resp) {
               console.log(resp)
...
<body>
  ...
  <script>
    ...
    util.fetch('https://api.apiopen.top/getJoke', null, true).then(function (resp) {
      var html = ''
      // 拼接li字符串显示在ul里
      resp.result.forEach(function (item) {
        html += `
    ...
    document.querySelector('#wrap').onclick = function (e) {
      ...
      if (target.tagName === 'BUTTON') {
        ...
        util.fetch('https://api.apiopen.top/getSingleJoke', { sid: sid }, true).then(function (resp) {
          var name = resp.result.name
          // 找到父级li，插入作者信息
          var b = document.createElement('b')
          b.innerHTML = name
...
- 上面别人使用的开放接口，别人就设置了cors的，所以可以直接访问
