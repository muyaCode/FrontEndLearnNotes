...
**安装ajv库**：[ajv - npm (npmjs.com)](https://www.npmjs.com/package/ajv)
...
### 2.在路由中使用：controllers控制器目录-处理路由请求参数数据
...
#### 最终在路由router/user.js导入userController模块方法并使用
...
## 原生Node全局变量来存储用户登录后生成的cookies

### 用户前端登录后，安全的设置存储cookies：router/user.js
...
## Redis数据库的使用：用来存储用户登录后生成的Session

Redis的npm文档：[redis - npm (npmjs.com)](https://www.npmjs.com/package/redis)
...
### 4.使用Redis数据库来存储用户登录后生成的Session

#### app.js：封装请求时候的Session判断
...
**sql语句的注释**：'-- '，双横杠+空格：注释掉密码输入的sql语句的条件
...
#### 使用预防SQL注入的方法escape：service/userService.js
...
## sequelize库替换mysql库 - 来操作mysql数据库
...
orm封装好了sql语句，不用再sql语句

### 1.安装sequelize和mysql2

Sequelize的npm文档：[sequelize - npm (npmjs.com)](https://www.npmjs.com/package/sequelize)

sequelize官网：[Sequelize | Feature-rich ORM for modern TypeScript & JavaScript](https://sequelize.org/)

sequelize中文网：[Sequelize 简介 | Sequelize 中文文档 | Sequelize 中文网](https://www.sequelize.cn/)
...
mysql2的npm文档：[mysql2 - npm (npmjs.com)](https://www.npmjs.com/package/mysql2)

```bash
npm install --save mysql2
```

### 2.sequelize连接mysql数据库：db/seq.js
...
2.sequelize在根据模型创建表的时候, 会自动增加两个字段 createAt/updateAt
* */
let User = seq.define('user', { // 创建user表
    id: {
        type: Sequelize.INTEGER,
        primaryKey: true,
        autoIncrement: true
...
### 2.日志方法在app.js中注入日志监控：writeLog方法
