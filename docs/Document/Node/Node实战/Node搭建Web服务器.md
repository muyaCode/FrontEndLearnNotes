...
server.on("request", (req, res) => {
	...
	if (method === "get") {
		...
		if (path === "/login") {
			...
			// 1.告诉客户端返回的数据是什么类型
			res.writeHead(200, {
				// 'Content-Type': 'text/plain; charset=utf-8;' // utf-8数据类型
...
### 静态资源服务，文件：staticServer.js

```js
const path = require("path");
const fs = require("fs");
const mime = require("./mime"); // mime文件后缀定义
/**
 * 读取静态资源
 * @param req  请求对象
 * @param res  响应对象
...
### app.js 中，请求先判断是否静态资源：staticServer.readFile(req, res, rootPath)，再设置其他请求响应数据处理
...
// 静态资源加载模块
const rootPath = path.join(__dirname, "www"); // www目录放置静态资源文件
...
ejs 的 npm 文档：[ejs - npm (npmjs.com)](https://www.npmjs.com/package/ejs)

### 加载 ejs 模板引擎，文件：dynamicServer.js
...
### 使用 ejs 模板编写 html：views\index.html
